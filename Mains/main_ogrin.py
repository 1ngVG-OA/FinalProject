"""End-to-end forecasting pipeline for the ogrin series.

This module is fully self-contained: every modelling choice, hyperparameter
grid, and preprocessing step for the ogrin series lives here.
...
Produced artifacts:
    - results/metrics_summary_ogrin.csv
    - results/best_models_params_ogrin.json
    - results/plots/ogrin_*.png
"""
...
# Modify the constants below to customise the ogrin analysis without
# touching any other series pipeline.

_SERIES_NAME = "ogrin"
_CFG = SERIES_CONFIG[_SERIES_NAME]

# To use a different grid for ogrin only, replace the right-hand side.
_MLP_PARAM_GRID = MLP_PARAM_GRID
_XGB_PARAM_GRID = XGB_PARAM_GRID
_MLP_EPOCHS = MLP_EPOCHS


def _run_ogrin_pipeline() -> tuple[pd.DataFrame, dict]:
    """Execute the full forecasting workflow for the ogrin series.
    ...
    4) ARMA: auto_arima without seasonality for daily non-seasonal data.
...
def run_pipeline_ogrin() -> tuple[pd.DataFrame, dict]:
    """Run and persist the ogrin forecasting pipeline.
    ...
    metrics_df, params = _run_ogrin_pipeline()
    metrics_df = metrics_df.sort_values(["series", "test_rmse"]).reset_index(drop=True)
    metrics_df.to_csv(RESULTS_DIR / "metrics_summary_ogrin.csv", index=False)

    report = {_SERIES_NAME: params}
    with open(RESULTS_DIR / "best_models_params_ogrin.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, default=str)

    print(f"Saved: {RESULTS_DIR / 'metrics_summary_ogrin.csv'}")
    print(f"Saved: {RESULTS_DIR / 'best_models_params_ogrin.json'}")
